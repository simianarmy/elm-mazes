...
createGrid rows cols initSeed cellMaker =
    let
        mask_ =
            Mask.createMask cols rows
    in
    createGridFromMask mask_ initSeed cellMaker
    ...
    let
        createMaskedCell row col =
            if Mask.get mask row col then
                ...
                RectCellTag (Cell.createMaskedCell row col)
...
toAscii grid cellViewer =
    let
        cellToString : GridCell -> RowAscii -> RowAscii
        cellToString cell ascii =
            let
                ...
                east_boundary =
                    if isLinkedTo cell (east grid bcell) then
                        " "
                ...
                south_boundary =
                    if isLinkedTo cell (south grid bcell) then
                        "   "
        ...
        rowToStrings row =
            let
                ...
                finalascii =
                    List.foldl cellToString rowascii cells
            in
            finalascii.top ++ "\n" ++ finalascii.bottom ++ "\n"
    in
    "+"
...
painter grid cellPainter cellSize cellInset =
    let
        ...
        ox =
            toFloat (negate imgWidth) / 2.0

        oy =
            toFloat imgHeight / 2.0

        inset =
            round (toFloat cellSize * cellInset)
        ...
        cellWallsWithInset style gridcell { x1, x2, x3, x4, y1, y2, y3, y4 } =
            ...
            if cell.masked then
                ...
                List.map (traced style) <|
                    List.concat
                        [ if isLinkedTo gridcell (north grid cell) then
                            [ segment ( x2, y1 ) ( x2, y2 ), segment ( x3, y1 ) ( x3, y2 ) ]

                          else
                            [ segment ( x2, y2 ) ( x3, y2 ) ]
                        , if isLinkedTo gridcell (south grid cell) then
                            [ segment ( x2, y3 ) ( x2, y4 ), segment ( x3, y3 ) ( x3, y4 ) ]

                          else
                            [ segment ( x2, y3 ) ( x3, y3 ) ]
                        , if isLinkedTo gridcell (west grid cell) then
                            [ segment ( x1, y2 ) ( x2, y2 ), segment ( x1, y3 ) ( x2, y3 ) ]

                          else
                            [ segment ( x2, y2 ) ( x2, y3 ) ]
                        , if isLinkedTo gridcell (east grid cell) then
                            [ segment ( x3, y2 ) ( x4, y2 ), segment ( x3, y3 ) ( x4, y3 ) ]
        ...
        cellBackground style gc =
            let
                ...
                bgRect =
                    filled (cellPainter gc) (cellToRect cell)

                --dbg = outlinedText style (Text.fromString " C ")
                halfSize =
                    toFloat cellSize / 2.0

                cx =
                    toFloat (cell.col * cellSize) + halfSize

                cy =
                    toFloat (negate cell.row * cellSize) - halfSize
            in
            move ( cx, cy ) bgRect
        ...
        cellBackgroundWithInset style gc { x1, x2, x3, x4, y1, y2, y3, y4 } =
            let
                ...
                fillfn =
                    filled (cellPainter gc)

                middleRect =
                    fillfn <| rect (x3 - x2) (y3 - y2)

                rects =
                    middleRect
                        :: List.concat
                            [ if isLinkedTo gc (north grid cell) then
                                [ moveY (halfInsetHeight + halfInset) <| fillfn <| rect (x3 - x2) (abs (y2 - y1)) ]

                              else
                                []
                            , if isLinkedTo gc (south grid cell) then
                                [ moveY (negate (halfInsetHeight + halfInset)) <| fillfn <| rect (x3 - x2) (abs (y4 - y3)) ]

                              else
                                []
                            , if isLinkedTo gc (west grid cell) then
                                [ moveX (negate (halfInsetWidth + halfInset)) <| fillfn <| rect (x2 - x1) (abs (y3 - y2)) ]

                              else
                                []
                            , if isLinkedTo gc (east grid cell) then
                                [ moveX (halfInsetWidth + halfInset) <| fillfn <| rect (x4 - x3) (abs (y3 - y2)) ]
                ...
                forms =
                    if inset > 0 then
                        let
                            ...
                            walls =
                                cellWallsWithInset style gcell insetCoords

                            bg =
                                cellBackgroundWithInset style gcell insetCoords
                        in
                        bg :: walls

                    else
                        cellBackground style gcell :: cellWalls style gcell x y
            in
            group forms

        -- cast grid.cells to rectCells
        drawables =
            List.map paintCell (cellsList grid.cells)
    in
...
cellsListToCellGrid cells =
    ...
    let
        rowMax =
            Maybe.withDefault 0 <| List.maximum <| List.map GridCell.row cells
    in
...
getCell :
    { a | cells : CellGrid, rows : Int, cols : Int }
    -> Int
    -> Int
    -> Maybe GridCell
getCell grid row col =
    -- validate bounds
    if row >= grid.rows || row < 0 || col < 0 then
        Nothing

    else
        let
            rowCells =
                Maybe.withDefault Array.empty <| Array.get row grid.cells

            -- Get a cell to check it's type...lame I know
            sampleCell =
                Array.get 0 rowCells
        in
        case sampleCell of
            Just (RectCellTag c) ->
                if (col >= grid.cols) || c.masked then
                    Nothing

                else
                    Array.get col rowCells
            ...
            Just (PolarCellTag ( c, o )) ->
                ...
                let
                    rowLen =
                        Array.length rowCells

                    -- TODO: mask check
                in
                Array.get (modBy rowLen col) rowCells
...
randomCell grid =
    let
        ( row, col ) =
            Mask.randomLocation grid.mask grid.rnd
    in
    getCell grid row col
...
braid grid neighborsFn p =
    let
        ...
        linkNeighbor g deadEnd =
            ...
            let
                neighbors =
                    filterNeighbors2 neighborsFn (\c -> not <| Cell.isLinked (GridCell.base deadEnd) (GridCell.base c)) g deadEnd

                -- select best neighbor
                best =
                    List.filter (\c -> Set.size (GridCell.links c) == 1) neighbors

                best_ =
                    if List.isEmpty best then
                        neighbors
                ...
                neighbor =
                    maybeGridCellToGridCell <| GridUtils.sampleCell best_ g.rnd

                g_ =
                    updateRnd g
            in
            if Cell.isNilCellID (GridCell.id neighbor) then
                Debug.crash "NIL NEIGHBOR in braid:linkNeighbor!" g_

            else
                linkCells g_ deadEnd neighbor True

        processDeadEnd : GridCell -> Grid -> Grid
        processDeadEnd deadEnd g =
            let
                ( randp, _ ) =
                    Random.step randpGen g.rnd.seed

                g_ =
                    updateRnd g
            in
            if (randp > p) || (not <| Set.size (GridCell.links deadEnd) == 1) then
                g_

            else
                linkNeighbor g_ deadEnd

        randomDeadEnds =
            Tuple.first <| ListUtils.shuffle (deadEnds grid) grid.rnd.seed
...
linkCellsHelper grid cell cellToLink =
    let
        linkCell : BaseCell -> BaseCell -> BaseCell
        ...
        linkMatched c =
            ...
            case c of
                RectCellTag rc ->
                    RectCellTag (linker rc)

                PolarCellTag ( pc, data ) ->
                    PolarCellTag ( linker pc, data )

                HexCellTag rc ->
                    HexCellTag (linker rc)

                TriangleCellTag rc ->
                    TriangleCellTag (linker rc)

                OuterCellTag rc ->
                    OuterCellTag (linker rc)
    in
...
linkCells grid cell cell2 bidi =
    let
        b1 =
            GridCell.base <| Debug.log "Linking" cell

        b2 =
            GridCell.base <| Debug.log "to" cell2

        grid_ =
            linkCellsHelper grid b1 b2
    in
    if bidi then
        linkCellsHelper grid_ b2 b1
...
cellToAscii grid cell =
    if (GridCell.base cell).masked then
        "M"
