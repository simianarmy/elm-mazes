...
cellDistances dgrid root =
    ...
    let
        weights =
            Distances.init (GridCell.base root)
        ...
        scanCellLinks neighbor acc =
            let
                ncell =
                    GridCell.base neighbor

                totalWeight =
                    Distances.lookup acc.weights (GridCell.base acc.curCell) + ncell.weight

                nWeight =
                    Distances.lookup acc.weights ncell
            in
            if (nWeight < 0) || (totalWeight < nWeight) then
                { acc
        ...
        pendingAcc acc =
            if List.isEmpty acc.pending then
                acc

            else
                let
                    sortedByWeight =
                        List.sortBy (\c -> (GridCell.base c).weight) acc.pending

                    cell =
                        GridCell.maybeGridCellToGridCell <| List.head sortedByWeight
