...
update msg model =
    case msg of
        ...
        UpdateWidth str ->
            let
                maze_ =
                    Maze.updateSize model.maze (String.toInt str |> Result.toMaybe |> Maybe.withDefault model.maze.grid.cols) model.maze.grid.rows
            in
            ( { model | maze = maze_ }
            , Cmd.none
            )
        ...
        UpdateHeight str ->
            let
                maze_ =
                    Maze.updateSize model.maze model.maze.grid.cols (String.toInt str |> Result.toMaybe |> Maybe.withDefault model.maze.grid.rows)
            in
            ( { model | maze = maze_ }
            , Cmd.none
            )
        ...
        SelectAlg str ->
            let
                ...
                maze_ =
                    Maze.init (Maze.algByName str) maze.grid.cols maze.grid.rows maze.grid.rnd.seed maze.shape maze.display
            in
            ( { model | maze = maze_ }
            , Cmd.none
            )
        ...
        SelectView display ->
            let
                maze =
                    Maze.updateView model.maze <| Debug.log "display" (displayFromString display)
            in
            ( { model | maze = maze }
            , Cmd.none
            )
        ...
        SelectShape shape ->
            ...
            let
                ...
                maze_ =
                    Maze.init maze.alg maze.grid.cols maze.grid.rows maze.grid.rnd.seed (shapeFromString shape) maze.display
            in
            ( { model | maze = maze_ }
            , Cmd.none
            )
        ...
        Braid act ->
            let
                factor =
                    Result.withDefault Maze.defaultBraidFactor (String.toFloat model.braidSlider.value)

                maze_ =
                    Maze.updateBraiding model.maze factor
            in
            ( { model
                | maze = maze_
        ...
        LoadImageMask png ->
            let
                mask =
                    Mask.fromImage ( png.width, png.height ) png.blackFlags
            in
            ( { model | maze = Maze.setMask model.maze mask }
            , Cmd.none
            )
        ...
        Tick dt ->
            ...
            let
                model_ =
                    if truncate model.totalTime >= mazeGenStepTime then
                        { model
...
view model =
    ...
    div [ id "main" ]
        ...
        , input
            [ class "sizeInput"
            , HA.value (toString maze.grid.cols)
        ...
        , input
            [ class "sizeInput"
            , HA.value (toString maze.grid.rows)
...
displayFromString str =
    ...
    else if str == "Colored" then
        Maze.Colored

    else if str == "Weighted" then
        Maze.Weighted
...
shapeFromString str =
    let
        s =
            List.filter (\e -> Tuple.second e == str) Maze.shapes
    in
    case List.head s of
        Nothing ->
            initShape
