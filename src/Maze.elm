    ...
    let
        ...
        cellGenFn =
            case shape of
                ...
                Triangle ->
                    TriangleGrid.makeCells

        grid_ =
            Grid.createGridFromMask mask seed cellGenFn
    in
    { grid = grid_
...
braid maze =
    let
        bgrid =
            Grid.braid maze.grid (neighborsFn maze.shape) maze.braidFactor
    in
    { maze
        | grid = bgrid
    }
...
update maze step =
    ...
    let
        ...
        grid_ =
            maze.generator maze.grid maze.step
    in
    { maze
        | grid = grid_
...
setMask maze mask =
    let
        grid_ =
            Grid.createGridFromMask mask maze.grid.rnd.seed maze.grid.cellMaker

        -- we want to run the algorithm to completion now
    in
    { maze | grid = maze.generator grid_ (Grid.size maze.grid) }
...
viewDistances maze =
    let
        center =
            Grid.center maze.grid

        start =
            center

        goal =
            GridCell.maybeGridCellToGridCell <| getCell maze.grid 0 0

        dgrid =
            DistanceGrid.createGrid maze.grid center

        pathDistances =
            DistanceGrid.pathTo dgrid start goal

        shortestPathGrid =
            { dgrid | dists = pathDistances }

        longDistances =
            DistanceGrid.longestPath dgrid center

        longestPathGrid =
            { dgrid | dists = longDistances }

        rootStr =
            GridCell.toString center
    in
    div []
        [ br [] []
...
viewWeightedDistances maze =
    let
        start =
            GridCell.maybeGridCellToGridCell <| Grid.getCell maze.grid 0 0

        finish =
            GridCell.maybeGridCellToGridCell <| getCell maze.grid (maze.grid.rows - 1) (maze.grid.cols - 1)

        wgrid =
            WeightedGrid.createGrid maze.grid start

        pathDistances =
            WeightedGrid.pathTo wgrid finish

        shortestPathGrid =
            { wgrid | dists = pathDistances }
    in
    div []
        ...
        , text <|
            "Shortest path from "
                ++ GridCell.toString start
                ++ " to :"
                ++ GridCell.toString finish
                ++ " Distance: "
                ++ (toString <| Dict.size pathDistances.cells)
        ...
        , if not <| isGenerated maze then
            ...
            let
                path =
                    Distances.cells pathDistances

                midCellID =
                    List.head <| List.reverse <| List.take (List.length path // 2) path

                midCell =
                    case midCellID of
                        Just d ->
                            Grid.cellIdToCell wgrid.dgrid.grid d
...
viewWeightedDistancesWithLava grid start goal lava =
    let
        lava_ =
            GridCell.setWeight lava 50

        lavaGrid =
            Grid.updateCellById grid (GridCell.id lava_) lava_

        lavaDGrid =
            WeightedGrid.createGrid lavaGrid start

        lavaPathDistances =
            WeightedGrid.pathTo lavaDGrid goal

        lavaPathGrid =
            { lavaDGrid | dists = lavaPathDistances }
    in
    div []
        [ text <|
            "Shortest path with lava from "
                ++ GridCell.toString start
                ++ " to :"
                ++ GridCell.toString goal
                ++ " Distance: "
                ++ (toString <| Dict.size lavaPathDistances.cells)
...
mazeToElement maze =
    let
        renderer =
            GridRenderer.toColoredElement maze.grid
    in
    case maze.shape of
        Rect ->
            let
...
algorithms shape =
    ...
    case shape of
        ...
        Polar ->
            List.concat [ algs, allAlgs ]

        Triangle ->
            List.concat [ algs, triangleAlgs, allAlgs ]

        _ ->
            List.concat [ algs, rectAlgs, allAlgs ]
...
algByName str =
    let
        res =
            List.head <| List.filter (\a -> a.name == str) (algorithms Rect)
    in
    case res of
        Just a ->
            a.alg
        ...
        _ ->
            Debug.crash "Unknown algorithm" BinaryTree
