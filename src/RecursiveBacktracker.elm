...
step gridOps grid i =
    ...
    if List.isEmpty <| grid.stack then
        if i > 0 then
            Debug.log "DONE!" grid
            -- we're done
            ...
            let
                current =
                    GridCell.maybeGridCellToGridCell <| gridOps.startCell grid
            in
            { grid
        ...
        case currentId of
            ...
            Just cid ->
                let
                    currentCell =
                        GridCell.maybeGridCellToGridCell <| Grid.getCellById grid cid
                in
...
work grid gridOps currentCell =
    let
        neighbors =
            Grid.filterNeighbors2 gridOps.neighbors (\c -> not <| Cell.hasLinks (GridCell.base c)) grid currentCell
    in
    -- is current cell at a dead end?
    if isEmpty neighbors then
        let
            stack_ =
                Maybe.withDefault [] (tail grid.stack)
        in
        { grid | stack = stack_ }
        ...
        let
            neighbor =
                GridUtils.sampleCell neighbors grid.rnd
                    |> GridCell.maybeGridCellToGridCell

            grid_ =
                gridOps.linkCells grid currentCell neighbor True
...
walkRandomly grid neighborsFn stack =
    if isEmpty stack then
        ...
        let
            current =
                head stack |> GridCell.maybeGridCellToGridCell
            ...
            neighbors =
                Grid.filterNeighbors2 neighborsFn (\c -> not <| Cell.hasLinks (GridCell.base c)) grid current
        in
        if isEmpty neighbors then
            jump (\() -> walkRandomly grid neighborsFn (Maybe.withDefault [] (tail stack)))
            ...
            let
                ...
                grid_ =
                    Grid.linkCells grid current neighbor True
