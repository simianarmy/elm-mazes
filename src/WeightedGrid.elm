...
createGrid grid start =
    let
        dg =
            DistanceGrid.createGrid grid start

        ds =
            DijkstraWeighted.cellDistances dg start
...
pathTo wgrid gcgoal =
    let
        ...
        breadcrumbs =
            Distances.add (Distances.init root) current (lookup wgrid.dists current)
        ...
        walkPath xpbreadcrumbs xpcurrent =
            if xpcurrent.id == root.id then
                ...
                let
                    links =
                        Grid.gridCellsToBaseCells <| Grid.linkedCells wgrid.dgrid.grid (RectCellTag xpcurrent)

                    currentDistance =
                        lookup wgrid.dists xpcurrent
                ...
                if List.isEmpty res then
                    ...
                    let
                        ...
                        ixpbreadcrumbs =
                            Distances.add xpbreadcrumbs neighbor (lookup wgrid.dists neighbor)
                    in
...
cellBackgroundColor wgrid gc =
    ...
    if bc.weight > 1 then
        ...
        let
            distance =
                Distances.lookup wgrid.dists bc
            ...
            intensity =
                64 + 191 * (wgrid.maximum - distance_) // wgrid.maximum
        in
        if distance == -1 then
            Color.white
