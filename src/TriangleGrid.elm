    ...
    let
        createMaskedCell row col =
            if Mask.get mask row col then
                TriangleCellTag (Cell.createCell row col)

            else
                TriangleCellTag (Cell.createMaskedCell row col)
...
neighbors grid gc =
    case gc of
        ...
        _ ->
            Debug.crash "Illegal call to HexGrid.neighbors with non-TriangleCellTag type cell"
...
painter grid cellPainter cellSize cellInset =
    let
        ...
        imgWidth =
            round (toFloat cellSize * (toFloat grid.cols + 1) / 2)

        imgHeight =
            round (height * toFloat grid.rows)
        ...
        cellBackground gc vx =
            let
                ...
                ngon =
                    GC.polygon [ ( vx.westX, vx.baseY ), ( vx.midX, vx.apexY ), ( vx.eastX, vx.baseY ) ]
        ...
        cellWalls style gc vx =
            let
                ...
                noSouth =
                    upright cell && (not <| GridCell.isValidCell (south grid cell))

                notLinked =
                    (not <| upright cell) && (not <| Cell.isLinked cell (maybeGridCellToCell (north grid cell)))
            in
            if cell.masked then
                []
        ...
        paintCell gc =
            let
                ...
                style =
                    { dl | width = 3, color = wallColor }

                cx =
                    halfWidth + toFloat cell.col * halfWidth

                cy =
                    halfHeight + toFloat cell.row * height
