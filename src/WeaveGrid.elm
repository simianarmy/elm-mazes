...
module WeaveGrid exposing (linkCells, neighbors)
...
neighbors grid gc =
    case gc of
        OuterCellTag c ->
            let
                checkNeighbor : Bool -> (Grid -> Cell -> Maybe GridCell) -> List GridCell
...
linkCells grid cell cell2 bidi =
    let
        ...
        checkEqualCells c1 c2 =
            if GridCell.toString c1 == GridCell.toString c2 then
                Just c1
    ...
    case findTunnelCell of
        Just n ->
            tunnelUnder grid cell n

        -- Call regular ol' linkCells from the Grid module
        Nothing ->
            Grid.linkCells grid cell cell2 bidi
