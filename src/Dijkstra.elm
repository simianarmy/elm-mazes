...
cellDistances grid cell =
    ...
    let
        ...
        scanCell cell linked diter =
            if not (Distances.lookup diter.dists linked == -1) then
                diter

            else
                let
                    curDist =
                        Distances.lookup diter.dists cell
                in
                { diter
        ...
        scanFrontier diter =
            let
                res =
                    List.foldl scanCellLinks diter diter.frontier
            in
        ...
        frontierAcc diter =
            if List.isEmpty diter.frontier then
                diter
