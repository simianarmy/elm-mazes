...
-- Does all the work of initializing a weave grid's cells
...
makeCells mask =
    let nrows = mask.rows
        ...
        rows_ = Array.set 0 (Array.fromList [GridCell.cellToWeaveCell (Cell.createCell 0 0)]) rows
            ...
            if row >= nrows
               ...
               let radius = (toFloat row) / (toFloat nrows)
                   ...
                   rowCells = Array.initialize ncells (\a -> 
                       GridCell.cellToWeaveCell (Cell.createCell row a)
                   )
...
configureCells rows cols incells =
    ...
    let cellList = Grid.cellsList incells
        ...
        configurer gc work =
            ...
            -- parent.outward << cell
            -- cell.inward = parent
            let (cell, _) = GridCell.toWeaveCell gc
...
toCellList cell =
    case cell of
        Nothing -> []
        Just cell -> [WeaveCellTag cell]

maybeGridCellToMaybeWeaveCell : Maybe GridCell -> Maybe (BaseCell, (CellID, CellLinks))
maybeGridCellToMaybeWeaveCell cell =
    Maybe.map GridCell.toWeaveCell cell
...
neighbors grid cell =
    case cell of
        WeaveCellTag (c, (inId, outwardIds)) ->
...
painter grid cellPainter cellSize cellInset =
    let imgSize = 2 * grid.rows * cellSize
        ...
        cellLines gc =
            let (cell, (inward, outwards)) = toWeaveCell gc
                ...
                linkedInward = Cell.isLinked cell (Tuple.first <| GridCell.toWeaveCell <| (cellIdToCell grid inward))
        ...
        drawables = List.concatMap cellLines <| 
            List.filter (\c -> (Tuple.first (toWeaveCell c)).row > 0) (Grid.cellsList grid.cells)
