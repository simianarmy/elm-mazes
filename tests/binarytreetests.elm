...
all : Test
all = describe "Binary Tree Algorithm test suite"
    [ test "Grid cells are of equal length" <| \() -> (
        let seed = Random.initialSeed 123123
            grid = TestHelpers.createGrid 3 3
        in
    ...
    , test "Cell top row links exist" <| \() -> (
        let grid = TestHelpers.createGrid 3 3
            gc00 = maybeGridCellToGridCell <| Grid.getCell grid 0 0
            gc01 = maybeGridCellToGridCell <| Grid.getCell grid 0 1
        in
    ...
    , test "Cell east edge links exist" <| \() -> (
        let grid = TestHelpers.createGrid 3 3
            gc20 = maybeGridCellToGridCell <| Grid.getCell grid 2 0
            gc21 = maybeGridCellToGridCell <| Grid.getCell grid 2 1
        in
