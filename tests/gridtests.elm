        ...
        , test "Linking cells bidirectionally retains linking state in cells" <|
            \() ->
                let
                    ...
                    oneone =
                        maybeGridCellToCell (getCell grid_ 0 1)
                in
        ...
        , test "Linking the same cell repeatedly maintains original links" <|
            \() ->
                let
                    ...
                    c1_ =
                        Debug.log "c1_" <| maybeGridCellToGridCell (getCell grid_ 1 1)

                    grid__ =
                        linkCells grid_ c1_ (createRectCell 1 2) True

                    oneone =
                        Debug.log "1,1" <| maybeGridCellToCell (getCell grid__ 1 1)
                in
        ...
        , test "Linking cells in a row removes their east/west wall" <|
            \() ->
                let
                    ...
                    bc2 =
                        maybeGridCellToCell <| east grid_ (GridCell.base c1)

                    bc1 =
                        maybeGridCellToCell <| west grid_ (GridCell.base c2)
                in
