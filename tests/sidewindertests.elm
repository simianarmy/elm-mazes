...
module Main exposing (main, sidewinderTests)
...
sidewinderTests =
    suite "Sidewinder Algorithm test suite"
        ...
        , test "Cell links are created"
            (let
                grid =
                    on (createGrid 3 3 (Random.initialSeed 123123))
             in
        ...
        , test "Cell top row eastern links exist"
            (let
                grid =
                    on (createGrid 3 3 (Random.initialSeed 123))
             in
             -- top row cells should all have an eastern link
             assert
                (Set.member "1:2" (toValidCell (getCell grid 1 1)).links
        ...
        , test "Cell east row north/south links exist"
            (let
             ...
             assert
                (Set.member "1:3" (toValidCell (getCell grid 2 3)).links
