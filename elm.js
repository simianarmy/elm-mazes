...
Elm.HuntAndKill.make = function (_elm) {
   ...
   var randomWalk = F3(function (grid,gcell,neighborsFn) {
      randomWalk: while (true) {
         ...
         if ($Cell.isNilCell(cell)) return grid; else {
               ...
               grid,
               gcell);
               if ($List.isEmpty(unvisitedNeighbors)) return grid; else {
                     var neighbor = $GridCell.maybeGridCellToGridCell(A2($GridUtils.sampleCell,unvisitedNeighbors,grid.rnd));
                     var grid$ = A4($Grid.linkCells,grid,gcell,neighbor,true);
                     var grid$$ = $Grid.updateRnd(grid$);
   ...
   var step = F4(function (startCellFn,neighborsFn,grid,i) {
      var visited = A2($GridCell.filterGridCells,function (e) {    return e.visited;},$Grid.cellsList(grid.cells));
...
Elm.Sidewinder.make = function (_elm) {
      ...
      var processCell = F2(function (ogCell,rowState) {
         if (rowState.stop) return rowState; else {
               ...
               if (shouldCloseOut) {
                  ...
                  $GridCell.setProcessed($GridCell.maybeGridCellToGridCell(A2($Grid.east,grid$,basecell))),
                  true)});
            }
      });
      return function (_) {
   ...
   var step = F4(function (startCellFn,neighborsFn,grid,i) {
      ...
      if (_p0.ctor === "Just") {
            ...
            function (c) {
               return $Basics.not($GridCell.base(c).processed);
            },
